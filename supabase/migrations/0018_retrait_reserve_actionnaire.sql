-- ============================================================================
--  DÉPÔT BOISSONS — MIGRATION 18 : RETRAIT DE LA RÉSERVE PRORATÉE (0017)
-- ----------------------------------------------------------------------------
--  La migration 17 déduisait une "réserve amortissement" du bénéfice net de
--  chaque actionnaire, au prorata de sa part. Ce n'est PAS ce qui était
--  demandé : l'amortissement du tricycle (5000 XOF/jour) doit être un montant
--  déclaré par le GÉRANT dans son point journalier, pas une réserve calculée
--  par actionnaire. On restaure get_compte_actionnaire / get_benefices_actionnaires
--  à leur version de la migration 16 (charge résiduelle uniquement, sans
--  réserve/amortissement).
--  La table `amortissements` (migration 17) n'est plus utilisée par ces RPC ;
--  elle est laissée en place (vide de conséquence) au cas où.
--  Idempotent.
-- ============================================================================
